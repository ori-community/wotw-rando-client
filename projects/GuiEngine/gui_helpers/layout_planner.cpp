#include <gui_helpers/layout_planner.h>
...
namespace gui_engine {
    ...
    int add_box(LayoutPlanner& layout, BoxType type, std::string const& name)
    {
    ...
    void update_size_members(LayoutPlanner& layout, Box& box)
    {
    ...
    void calculate_weighted_space(LayoutPlanner& layout, ImRect const& bounds)
    {
    ...
    void update_bounds(LayoutPlanner& layout, Box& box)
    {
        if (box.type == BoxType::Element)
            return;
    ...
    void update_positions(LayoutPlanner& layout)
    {
    ...
    ImRect get_bounds(LayoutPlanner const& layout, std::string const& name, ImVec4 const& margins)
    {
