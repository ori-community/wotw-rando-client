        ...
        void il2cpp_intercepts() {
            ...
            while (current != nullptr) {
                ...
                if (!current->nested.empty()) {
                    ...
                    if (klass == nullptr) {
                        trace(MessageType::Error, 3, "initialize", format("Error during il2cpp inject commit: failed to find klass '%s.%s' in namezpace '%s'", current->klass.data(), current->nested.data(), current->namezpace.data()));
                    ...
                    if (klass == nullptr) {
                        trace(MessageType::Error, 3, "initialize", format("Error during il2cpp inject commit: failed to find klass '%s' in namezpace '%s'", current->klass.data(), current->namezpace.data()));
                        current = current->prev;
                        continue;
                    }
                }
                ...
                if (method == nullptr) {
                    trace(MessageType::Error, 3, "initialize", format("Error during il2cpp inject commit: failed to find method '%s' in klass '%s' in namezpace '%s'", current->method_name.data(), current->klass.data(), current->namezpace.data()));
                    current = current->prev;
                    continue;
                }
...
#undef IL2CPP_TYPEDEF
        ...
        il2cpp_intercept::il2cpp_intercept(bool p_ztatic, std::string_view p_namezpace, std::string_view p_klass, std::string_view p_nested, std::string_view p_method_name, std::string p_params, std::string_view p_overload_params, void** p_original_pointer, void* p_intercept_pointer, int virtual_count) :
                ztatic(p_ztatic), namezpace(std::move(p_namezpace)), klass(std::move(p_klass)), nested(p_nested), overload_params(p_overload_params), method_name(std::move(p_method_name)), param_count(0), original_pointer(p_original_pointer), intercept_pointer(p_intercept_pointer), virtual_count(virtual_count), next(nullptr) {
