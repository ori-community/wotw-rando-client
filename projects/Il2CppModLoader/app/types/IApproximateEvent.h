#pragma once
#include <Il2CppModLoader/il2cpp_helpers.h>
#include <Il2CppModLoader/macros.h>

namespace app::classes::types {
    namespace IApproximateEvent {
        extern IL2CPP_MODLOADER_DLLEXPORT app::IApproximateEvent__Class** type_info;
        inline app::IApproximateEvent__Class* get_class() {
            return il2cpp::get_class<app::IApproximateEvent__Class>(type_info, "Moon.Timeline", "IApproximateEvent");
        }
        inline app::IApproximateEvent* create() {
            return il2cpp::create_object<app::IApproximateEvent>(get_class());
        }
    } // namespace IApproximateEvent
} // namespace app::classes::types
