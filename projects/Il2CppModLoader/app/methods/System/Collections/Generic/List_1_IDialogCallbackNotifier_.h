#pragma once
#include <Il2CppModLoader/interception_macros.h>

namespace app::methods::System::Collections::Generic::List_1_IDialogCallbackNotifier_ {
    IL2CPP_REGISTER_METHOD(0x02F1E150, void, Add, (app::List_1_IDialogCallbackNotifier_ * this_ptr, app::IDialogCallbackNotifier * item))
    IL2CPP_REGISTER_METHODINFO(0x04753438, List_1_IDialogCallbackNotifier__Add__MethodInfo)
    IL2CPP_REGISTER_METHOD(0x025F0B10, app::List_1_T_Enumerator_IDialogCallbackNotifier_, GetEnumerator, (app::List_1_IDialogCallbackNotifier_ * this_ptr))
    IL2CPP_REGISTER_METHODINFO(0x04787830, List_1_IDialogCallbackNotifier__GetEnumerator__MethodInfo)
    IL2CPP_REGISTER_METHOD(0x025E8100, void, ctor, (app::List_1_IDialogCallbackNotifier_ * this_ptr))
    IL2CPP_REGISTER_METHODINFO(0x0470F9A0, List_1_IDialogCallbackNotifier___ctor__MethodInfo)
}
