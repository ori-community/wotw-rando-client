#pragma once
#include <Il2CppModLoader/interception_macros.h>

namespace app::methods::System::Tuple_2_String_Single_ {
    IL2CPP_REGISTER_METHOD(0x02A68070, void, ctor, (app::Tuple_2_String_Single_ * this_ptr, app::String * item1, float item2))
    IL2CPP_REGISTER_METHODINFO(0x04793D40, Tuple_2_String_Single___ctor__MethodInfo)
    IL2CPP_REGISTER_METHOD(0x002FA280, app::String *, get_Item1, (app::Tuple_2_String_Single_ * this_ptr))
    IL2CPP_REGISTER_METHODINFO(0x04755238, Tuple_2_String_Single__get_Item1__MethodInfo)
    IL2CPP_REGISTER_METHOD(0x00863E90, float, get_Item2, (app::Tuple_2_String_Single_ * this_ptr))
    IL2CPP_REGISTER_METHODINFO(0x0478C098, Tuple_2_String_Single__get_Item2__MethodInfo)
}
