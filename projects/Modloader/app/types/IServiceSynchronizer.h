#pragma once
#include <Modloader/app/structs/IServiceSynchronizer.h>
#include <Modloader/app/structs/IServiceSynchronizer__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace IServiceSynchronizer {
        inline app::IServiceSynchronizer__Class** type_info() {
            static app::IServiceSynchronizer__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::IServiceSynchronizer__Class**)(modloader::win::memory::resolve_rva(0x04727338));
            }
            return cache;
        }
        inline app::IServiceSynchronizer__Class* get_class() {
            return il2cpp::get_class<app::IServiceSynchronizer__Class>(type_info(), "SystemIntegration.Synchronizer", "IServiceSynchronizer");
        }
    } // namespace IServiceSynchronizer
} // namespace app::classes::types
