#pragma once
#include <Modloader/app/structs/UberSwarmPOI_POIShape__Enum.h>
#include <Modloader/app/structs/UberSwarmPOI_POIShape__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace UberSwarmPOI_POIShape__Enum {
        inline app::UberSwarmPOI_POIShape__Enum__Class** type_info() {
            static app::UberSwarmPOI_POIShape__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::UberSwarmPOI_POIShape__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::UberSwarmPOI_POIShape__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::UberSwarmPOI_POIShape__Enum__Class>(type_info(), "", "UberSwarmPOI", "POIShape");
        }
        inline app::UberSwarmPOI_POIShape__Enum* create() {
            return il2cpp::create_object<app::UberSwarmPOI_POIShape__Enum>(get_class());
        }
    } // namespace UberSwarmPOI_POIShape__Enum
} // namespace app::classes::types
