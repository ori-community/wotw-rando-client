#pragma once
#include <Modloader/app/structs/LocalDataStore__Array.h>
#include <Modloader/app/structs/LocalDataStore__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace LocalDataStore__Array {
        inline app::LocalDataStore__Array__Class** type_info() {
            static app::LocalDataStore__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::LocalDataStore__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::LocalDataStore__Array__Class* get_class() {
            return il2cpp::get_class<app::LocalDataStore__Array__Class>(type_info(), "System", "LocalDataStore[]");
        }
        inline app::LocalDataStore__Array* create() {
            return il2cpp::create_object<app::LocalDataStore__Array>(get_class());
        }
    } // namespace LocalDataStore__Array
} // namespace app::classes::types
