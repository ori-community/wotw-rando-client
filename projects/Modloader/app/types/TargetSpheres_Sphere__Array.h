#pragma once
#include <Modloader/app/structs/TargetSpheres_Sphere__Array.h>
#include <Modloader/app/structs/TargetSpheres_Sphere__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace TargetSpheres_Sphere__Array {
        inline app::TargetSpheres_Sphere__Array__Class** type_info() {
            static app::TargetSpheres_Sphere__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::TargetSpheres_Sphere__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::TargetSpheres_Sphere__Array__Class* get_class() {
            return il2cpp::get_class<app::TargetSpheres_Sphere__Array__Class>(type_info(), "", "TargetSpheres+Sphere[]");
        }
        inline app::TargetSpheres_Sphere__Array* create() {
            return il2cpp::create_object<app::TargetSpheres_Sphere__Array>(get_class());
        }
    } // namespace TargetSpheres_Sphere__Array
} // namespace app::classes::types
