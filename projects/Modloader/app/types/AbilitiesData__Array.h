#pragma once
#include <Modloader/app/structs/AbilitiesData__Array.h>
#include <Modloader/app/structs/AbilitiesData__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace AbilitiesData__Array {
        inline app::AbilitiesData__Array__Class** type_info() {
            static app::AbilitiesData__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::AbilitiesData__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::AbilitiesData__Array__Class* get_class() {
            return il2cpp::get_class<app::AbilitiesData__Array__Class>(type_info(), "", "AbilitiesData[]");
        }
        inline app::AbilitiesData__Array* create() {
            return il2cpp::create_object<app::AbilitiesData__Array>(get_class());
        }
    } // namespace AbilitiesData__Array
} // namespace app::classes::types
