#pragma once
#include <Modloader/app/structs/LightmapBakeType__Enum.h>
#include <Modloader/app/structs/LightmapBakeType__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace LightmapBakeType__Enum {
        inline app::LightmapBakeType__Enum__Class** type_info() {
            static app::LightmapBakeType__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::LightmapBakeType__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::LightmapBakeType__Enum__Class* get_class() {
            return il2cpp::get_class<app::LightmapBakeType__Enum__Class>(type_info(), "UnityEngine", "LightmapBakeType");
        }
        inline app::LightmapBakeType__Enum* create() {
            return il2cpp::create_object<app::LightmapBakeType__Enum>(get_class());
        }
    } // namespace LightmapBakeType__Enum
} // namespace app::classes::types
