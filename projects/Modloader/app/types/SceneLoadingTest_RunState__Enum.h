#pragma once
#include <Modloader/app/structs/SceneLoadingTest_RunState__Enum.h>
#include <Modloader/app/structs/SceneLoadingTest_RunState__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SceneLoadingTest_RunState__Enum {
        inline app::SceneLoadingTest_RunState__Enum__Class** type_info() {
            static app::SceneLoadingTest_RunState__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SceneLoadingTest_RunState__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SceneLoadingTest_RunState__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::SceneLoadingTest_RunState__Enum__Class>(type_info(), "Moon", "SceneLoadingTest", "RunState");
        }
        inline app::SceneLoadingTest_RunState__Enum* create() {
            return il2cpp::create_object<app::SceneLoadingTest_RunState__Enum>(get_class());
        }
    } // namespace SceneLoadingTest_RunState__Enum
} // namespace app::classes::types
