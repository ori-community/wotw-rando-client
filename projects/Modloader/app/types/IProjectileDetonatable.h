#pragma once
#include <Modloader/app/structs/IProjectileDetonatable.h>
#include <Modloader/app/structs/IProjectileDetonatable__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace IProjectileDetonatable {
        inline app::IProjectileDetonatable__Class** type_info() {
            static app::IProjectileDetonatable__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::IProjectileDetonatable__Class**)(modloader::win::memory::resolve_rva(0x04728AB8));
            }
            return cache;
        }
        inline app::IProjectileDetonatable__Class* get_class() {
            return il2cpp::get_class<app::IProjectileDetonatable__Class>(type_info(), "", "IProjectileDetonatable");
        }
    } // namespace IProjectileDetonatable
} // namespace app::classes::types
