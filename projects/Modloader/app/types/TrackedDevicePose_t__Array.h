#pragma once
#include <Modloader/app/structs/TrackedDevicePose_t__Array.h>
#include <Modloader/app/structs/TrackedDevicePose_t__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace TrackedDevicePose_t__Array {
        inline app::TrackedDevicePose_t__Array__Class** type_info() {
            static app::TrackedDevicePose_t__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::TrackedDevicePose_t__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::TrackedDevicePose_t__Array__Class* get_class() {
            return il2cpp::get_class<app::TrackedDevicePose_t__Array__Class>(type_info(), "ZenFulcrum.VR.OpenVRBinding", "TrackedDevicePose_t[]");
        }
        inline app::TrackedDevicePose_t__Array* create() {
            return il2cpp::create_object<app::TrackedDevicePose_t__Array>(get_class());
        }
    } // namespace TrackedDevicePose_t__Array
} // namespace app::classes::types
