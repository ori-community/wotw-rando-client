#pragma once
#include <Modloader/app/structs/BEAUTIFY_PRESET__Enum.h>
#include <Modloader/app/structs/BEAUTIFY_PRESET__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace BEAUTIFY_PRESET__Enum {
        inline app::BEAUTIFY_PRESET__Enum__Class** type_info() {
            static app::BEAUTIFY_PRESET__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::BEAUTIFY_PRESET__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::BEAUTIFY_PRESET__Enum__Class* get_class() {
            return il2cpp::get_class<app::BEAUTIFY_PRESET__Enum__Class>(type_info(), "BeautifyEffect", "BEAUTIFY_PRESET");
        }
        inline app::BEAUTIFY_PRESET__Enum* create() {
            return il2cpp::create_object<app::BEAUTIFY_PRESET__Enum>(get_class());
        }
    } // namespace BEAUTIFY_PRESET__Enum
} // namespace app::classes::types
