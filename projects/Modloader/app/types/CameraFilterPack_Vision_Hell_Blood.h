#pragma once
#include <Modloader/app/structs/CameraFilterPack_Vision_Hell_Blood.h>
#include <Modloader/app/structs/CameraFilterPack_Vision_Hell_Blood__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace CameraFilterPack_Vision_Hell_Blood {
        inline app::CameraFilterPack_Vision_Hell_Blood__Class** type_info() {
            static app::CameraFilterPack_Vision_Hell_Blood__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::CameraFilterPack_Vision_Hell_Blood__Class**)(modloader::win::memory::resolve_rva(0x04754DF0));
            }
            return cache;
        }
        inline app::CameraFilterPack_Vision_Hell_Blood__Class* get_class() {
            return il2cpp::get_class<app::CameraFilterPack_Vision_Hell_Blood__Class>(type_info(), "", "CameraFilterPack_Vision_Hell_Blood");
        }
        inline app::CameraFilterPack_Vision_Hell_Blood* create() {
            return il2cpp::create_object<app::CameraFilterPack_Vision_Hell_Blood>(get_class());
        }
    } // namespace CameraFilterPack_Vision_Hell_Blood
} // namespace app::classes::types
