#pragma once
#include <Modloader/app/structs/WindowsInstallationType__Enum.h>
#include <Modloader/app/structs/WindowsInstallationType__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace WindowsInstallationType__Enum {
        inline app::WindowsInstallationType__Enum__Class** type_info() {
            static app::WindowsInstallationType__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::WindowsInstallationType__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::WindowsInstallationType__Enum__Class* get_class() {
            return il2cpp::get_class<app::WindowsInstallationType__Enum__Class>(type_info(), "System.Net", "WindowsInstallationType");
        }
        inline app::WindowsInstallationType__Enum* create() {
            return il2cpp::create_object<app::WindowsInstallationType__Enum>(get_class());
        }
    } // namespace WindowsInstallationType__Enum
} // namespace app::classes::types
