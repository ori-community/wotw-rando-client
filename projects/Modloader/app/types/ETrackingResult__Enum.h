#pragma once
#include <Modloader/app/structs/ETrackingResult__Enum.h>
#include <Modloader/app/structs/ETrackingResult__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace ETrackingResult__Enum {
        inline app::ETrackingResult__Enum__Class** type_info() {
            static app::ETrackingResult__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::ETrackingResult__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::ETrackingResult__Enum__Class* get_class() {
            return il2cpp::get_class<app::ETrackingResult__Enum__Class>(type_info(), "ZenFulcrum.VR.OpenVRBinding", "ETrackingResult");
        }
        inline app::ETrackingResult__Enum* create() {
            return il2cpp::create_object<app::ETrackingResult__Enum>(get_class());
        }
    } // namespace ETrackingResult__Enum
} // namespace app::classes::types
