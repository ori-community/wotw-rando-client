#pragma once
#include <Modloader/app/structs/CatalogItemContainerInfo_1.h>
#include <Modloader/app/structs/CatalogItemContainerInfo_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace CatalogItemContainerInfo_1 {
        inline app::CatalogItemContainerInfo_1__Class** type_info() {
            static app::CatalogItemContainerInfo_1__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::CatalogItemContainerInfo_1__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::CatalogItemContainerInfo_1__Class* get_class() {
            return il2cpp::get_class<app::CatalogItemContainerInfo_1__Class>(type_info(), "PlayFab.ServerModels", "CatalogItemContainerInfo");
        }
        inline app::CatalogItemContainerInfo_1* create() {
            return il2cpp::create_object<app::CatalogItemContainerInfo_1>(get_class());
        }
    } // namespace CatalogItemContainerInfo_1
} // namespace app::classes::types
