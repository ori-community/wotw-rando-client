#pragma once
#include <Modloader/app/structs/BitSet__Array.h>
#include <Modloader/app/structs/BitSet__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace BitSet__Array {
        inline app::BitSet__Array__Class** type_info() {
            static app::BitSet__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::BitSet__Array__Class**)(modloader::win::memory::resolve_rva(0x0475C448));
            }
            return cache;
        }
        inline app::BitSet__Array__Class* get_class() {
            return il2cpp::get_class<app::BitSet__Array__Class>(type_info(), "System.Xml.Schema", "BitSet[]");
        }
        inline app::BitSet__Array* create() {
            return il2cpp::create_object<app::BitSet__Array>(get_class());
        }
    } // namespace BitSet__Array
} // namespace app::classes::types
