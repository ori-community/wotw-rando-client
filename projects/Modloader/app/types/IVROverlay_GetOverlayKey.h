#pragma once
#include <Modloader/app/structs/IVROverlay_GetOverlayKey.h>
#include <Modloader/app/structs/IVROverlay_GetOverlayKey__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace IVROverlay_GetOverlayKey {
        inline app::IVROverlay_GetOverlayKey__Class** type_info() {
            static app::IVROverlay_GetOverlayKey__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::IVROverlay_GetOverlayKey__Class**)(modloader::win::memory::resolve_rva(0x04726078));
            }
            return cache;
        }
        inline app::IVROverlay_GetOverlayKey__Class* get_class() {
            return il2cpp::get_nested_class<app::IVROverlay_GetOverlayKey__Class>(type_info(), "ZenFulcrum.VR.OpenVRBinding", "IVROverlay", "_GetOverlayKey");
        }
        inline app::IVROverlay_GetOverlayKey* create() {
            return il2cpp::create_object<app::IVROverlay_GetOverlayKey>(get_class());
        }
    } // namespace IVROverlay_GetOverlayKey
} // namespace app::classes::types
