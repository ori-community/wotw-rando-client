#pragma once
#include <Modloader/app/structs/UserCustomIdInfo_1.h>
#include <Modloader/app/structs/UserCustomIdInfo_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace UserCustomIdInfo_1 {
        inline app::UserCustomIdInfo_1__Class** type_info() {
            static app::UserCustomIdInfo_1__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::UserCustomIdInfo_1__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::UserCustomIdInfo_1__Class* get_class() {
            return il2cpp::get_class<app::UserCustomIdInfo_1__Class>(type_info(), "PlayFab.ServerModels", "UserCustomIdInfo");
        }
        inline app::UserCustomIdInfo_1* create() {
            return il2cpp::create_object<app::UserCustomIdInfo_1>(get_class());
        }
    } // namespace UserCustomIdInfo_1
} // namespace app::classes::types
