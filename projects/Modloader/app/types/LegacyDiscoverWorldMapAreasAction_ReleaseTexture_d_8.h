#pragma once
#include <Modloader/app/structs/LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8.h>
#include <Modloader/app/structs/LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8 {
        inline app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class** type_info() {
            static app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class**)(modloader::win::memory::resolve_rva(0x04754678));
            }
            return cache;
        }
        inline app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class* get_class() {
            return il2cpp::get_nested_class<app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8__Class>(type_info(), "", "LegacyDiscoverWorldMapAreasAction", "<ReleaseTexture>d__8");
        }
        inline app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8* create() {
            return il2cpp::create_object<app::LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8>(get_class());
        }
    } // namespace LegacyDiscoverWorldMapAreasAction_ReleaseTexture_d_8
} // namespace app::classes::types
