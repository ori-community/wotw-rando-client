#pragma once
#include <Modloader/app/structs/MistyWoodsAreaMapCanvas_CanvasItem__Array.h>
#include <Modloader/app/structs/MistyWoodsAreaMapCanvas_CanvasItem__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace MistyWoodsAreaMapCanvas_CanvasItem__Array {
        inline app::MistyWoodsAreaMapCanvas_CanvasItem__Array__Class** type_info() {
            static app::MistyWoodsAreaMapCanvas_CanvasItem__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::MistyWoodsAreaMapCanvas_CanvasItem__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::MistyWoodsAreaMapCanvas_CanvasItem__Array__Class* get_class() {
            return il2cpp::get_class<app::MistyWoodsAreaMapCanvas_CanvasItem__Array__Class>(type_info(), "", "MistyWoodsAreaMapCanvas+CanvasItem[]");
        }
        inline app::MistyWoodsAreaMapCanvas_CanvasItem__Array* create() {
            return il2cpp::create_object<app::MistyWoodsAreaMapCanvas_CanvasItem__Array>(get_class());
        }
    } // namespace MistyWoodsAreaMapCanvas_CanvasItem__Array
} // namespace app::classes::types
