#pragma once
#include <Modloader/app/structs/CameraFilterPack_FX_8bits_gb.h>
#include <Modloader/app/structs/CameraFilterPack_FX_8bits_gb__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace CameraFilterPack_FX_8bits_gb {
        inline app::CameraFilterPack_FX_8bits_gb__Class** type_info() {
            static app::CameraFilterPack_FX_8bits_gb__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::CameraFilterPack_FX_8bits_gb__Class**)(modloader::win::memory::resolve_rva(0x04763F50));
            }
            return cache;
        }
        inline app::CameraFilterPack_FX_8bits_gb__Class* get_class() {
            return il2cpp::get_class<app::CameraFilterPack_FX_8bits_gb__Class>(type_info(), "", "CameraFilterPack_FX_8bits_gb");
        }
        inline app::CameraFilterPack_FX_8bits_gb* create() {
            return il2cpp::create_object<app::CameraFilterPack_FX_8bits_gb>(get_class());
        }
    } // namespace CameraFilterPack_FX_8bits_gb
} // namespace app::classes::types
