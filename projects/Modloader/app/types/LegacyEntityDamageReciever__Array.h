#pragma once
#include <Modloader/app/structs/LegacyEntityDamageReciever__Array.h>
#include <Modloader/app/structs/LegacyEntityDamageReciever__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace LegacyEntityDamageReciever__Array {
        inline app::LegacyEntityDamageReciever__Array__Class** type_info() {
            static app::LegacyEntityDamageReciever__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::LegacyEntityDamageReciever__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::LegacyEntityDamageReciever__Array__Class* get_class() {
            return il2cpp::get_class<app::LegacyEntityDamageReciever__Array__Class>(type_info(), "", "LegacyEntityDamageReciever[]");
        }
        inline app::LegacyEntityDamageReciever__Array* create() {
            return il2cpp::create_object<app::LegacyEntityDamageReciever__Array>(get_class());
        }
    } // namespace LegacyEntityDamageReciever__Array
} // namespace app::classes::types
