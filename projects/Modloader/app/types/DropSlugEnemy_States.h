#pragma once
#include <Modloader/app/structs/DropSlugEnemy_States.h>
#include <Modloader/app/structs/DropSlugEnemy_States__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DropSlugEnemy_States {
        inline app::DropSlugEnemy_States__Class** type_info() {
            static app::DropSlugEnemy_States__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::DropSlugEnemy_States__Class**)(modloader::win::memory::resolve_rva(0x047109B0));
            }
            return cache;
        }
        inline app::DropSlugEnemy_States__Class* get_class() {
            return il2cpp::get_nested_class<app::DropSlugEnemy_States__Class>(type_info(), "", "DropSlugEnemy", "States");
        }
        inline app::DropSlugEnemy_States* create() {
            return il2cpp::create_object<app::DropSlugEnemy_States>(get_class());
        }
    } // namespace DropSlugEnemy_States
} // namespace app::classes::types
