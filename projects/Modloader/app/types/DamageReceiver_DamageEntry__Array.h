#pragma once
#include <Modloader/app/structs/DamageReceiver_DamageEntry__Array.h>
#include <Modloader/app/structs/DamageReceiver_DamageEntry__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DamageReceiver_DamageEntry__Array {
        inline app::DamageReceiver_DamageEntry__Array__Class** type_info() {
            static app::DamageReceiver_DamageEntry__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::DamageReceiver_DamageEntry__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::DamageReceiver_DamageEntry__Array__Class* get_class() {
            return il2cpp::get_class<app::DamageReceiver_DamageEntry__Array__Class>(type_info(), "Moon", "DamageReceiver+DamageEntry[]");
        }
        inline app::DamageReceiver_DamageEntry__Array* create() {
            return il2cpp::create_object<app::DamageReceiver_DamageEntry__Array>(get_class());
        }
    } // namespace DamageReceiver_DamageEntry__Array
} // namespace app::classes::types
