#pragma once
#include <Modloader/app/structs/IKSolver__Array.h>
#include <Modloader/app/structs/IKSolver__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace IKSolver__Array {
        inline app::IKSolver__Array__Class** type_info() {
            static app::IKSolver__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::IKSolver__Array__Class**)(modloader::win::memory::resolve_rva(0x04795B20));
            }
            return cache;
        }
        inline app::IKSolver__Array__Class* get_class() {
            return il2cpp::get_class<app::IKSolver__Array__Class>(type_info(), "RootMotion.FinalIK", "IKSolver[]");
        }
        inline app::IKSolver__Array* create() {
            return il2cpp::create_object<app::IKSolver__Array>(get_class());
        }
    } // namespace IKSolver__Array
} // namespace app::classes::types
