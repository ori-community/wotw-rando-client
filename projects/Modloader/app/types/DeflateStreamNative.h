#pragma once
#include <Modloader/app/structs/DeflateStreamNative.h>
#include <Modloader/app/structs/DeflateStreamNative__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DeflateStreamNative {
        inline app::DeflateStreamNative__Class** type_info() {
            static app::DeflateStreamNative__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::DeflateStreamNative__Class**)(modloader::win::memory::resolve_rva(0x04757450));
            }
            return cache;
        }
        inline app::DeflateStreamNative__Class* get_class() {
            return il2cpp::get_class<app::DeflateStreamNative__Class>(type_info(), "System.IO.Compression", "DeflateStreamNative");
        }
        inline app::DeflateStreamNative* create() {
            return il2cpp::create_object<app::DeflateStreamNative>(get_class());
        }
    } // namespace DeflateStreamNative
} // namespace app::classes::types
