#pragma once
#include <Modloader/app/structs/DataColumnChangeEventHandler.h>
#include <Modloader/app/structs/DataColumnChangeEventHandler__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DataColumnChangeEventHandler {
        inline app::DataColumnChangeEventHandler__Class** type_info() {
            static app::DataColumnChangeEventHandler__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::DataColumnChangeEventHandler__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::DataColumnChangeEventHandler__Class* get_class() {
            return il2cpp::get_class<app::DataColumnChangeEventHandler__Class>(type_info(), "System.Data", "DataColumnChangeEventHandler");
        }
        inline app::DataColumnChangeEventHandler* create() {
            return il2cpp::create_object<app::DataColumnChangeEventHandler>(get_class());
        }
    } // namespace DataColumnChangeEventHandler
} // namespace app::classes::types
