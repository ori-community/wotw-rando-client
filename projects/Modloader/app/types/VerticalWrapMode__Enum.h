#pragma once
#include <Modloader/app/structs/VerticalWrapMode__Enum.h>
#include <Modloader/app/structs/VerticalWrapMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace VerticalWrapMode__Enum {
        inline app::VerticalWrapMode__Enum__Class** type_info() {
            static app::VerticalWrapMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::VerticalWrapMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::VerticalWrapMode__Enum__Class* get_class() {
            return il2cpp::get_class<app::VerticalWrapMode__Enum__Class>(type_info(), "UnityEngine", "VerticalWrapMode");
        }
        inline app::VerticalWrapMode__Enum* create() {
            return il2cpp::create_object<app::VerticalWrapMode__Enum>(get_class());
        }
    } // namespace VerticalWrapMode__Enum
} // namespace app::classes::types
