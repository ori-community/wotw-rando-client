#pragma once
#include <Modloader/app/structs/SetupStateModifier__Array.h>
#include <Modloader/app/structs/SetupStateModifier__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SetupStateModifier__Array {
        inline app::SetupStateModifier__Array__Class** type_info() {
            static app::SetupStateModifier__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SetupStateModifier__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SetupStateModifier__Array__Class* get_class() {
            return il2cpp::get_class<app::SetupStateModifier__Array__Class>(type_info(), "", "SetupStateModifier[]");
        }
        inline app::SetupStateModifier__Array* create() {
            return il2cpp::create_object<app::SetupStateModifier__Array>(get_class());
        }
    } // namespace SetupStateModifier__Array
} // namespace app::classes::types
