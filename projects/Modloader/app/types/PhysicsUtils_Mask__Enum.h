#pragma once
#include <Modloader/app/structs/PhysicsUtils_Mask__Enum.h>
#include <Modloader/app/structs/PhysicsUtils_Mask__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace PhysicsUtils_Mask__Enum {
        inline app::PhysicsUtils_Mask__Enum__Class** type_info() {
            static app::PhysicsUtils_Mask__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::PhysicsUtils_Mask__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::PhysicsUtils_Mask__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::PhysicsUtils_Mask__Enum__Class>(type_info(), "", "PhysicsUtils", "Mask");
        }
        inline app::PhysicsUtils_Mask__Enum* create() {
            return il2cpp::create_object<app::PhysicsUtils_Mask__Enum>(get_class());
        }
    } // namespace PhysicsUtils_Mask__Enum
} // namespace app::classes::types
