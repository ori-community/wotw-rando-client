#pragma once
#include <Modloader/app/structs/Capture__Array.h>
#include <Modloader/app/structs/Capture__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace Capture__Array {
        inline app::Capture__Array__Class** type_info() {
            static app::Capture__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::Capture__Array__Class**)(modloader::win::memory::resolve_rva(0x0470BC20));
            }
            return cache;
        }
        inline app::Capture__Array__Class* get_class() {
            return il2cpp::get_class<app::Capture__Array__Class>(type_info(), "System.Text.RegularExpressions", "Capture[]");
        }
        inline app::Capture__Array* create() {
            return il2cpp::create_object<app::Capture__Array>(get_class());
        }
    } // namespace Capture__Array
} // namespace app::classes::types
