#pragma once
#include <Modloader/app/structs/DofAnimator__Array.h>
#include <Modloader/app/structs/DofAnimator__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DofAnimator__Array {
        inline app::DofAnimator__Array__Class** type_info() {
            static app::DofAnimator__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::DofAnimator__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::DofAnimator__Array__Class* get_class() {
            return il2cpp::get_class<app::DofAnimator__Array__Class>(type_info(), "", "DofAnimator[]");
        }
        inline app::DofAnimator__Array* create() {
            return il2cpp::create_object<app::DofAnimator__Array>(get_class());
        }
    } // namespace DofAnimator__Array
} // namespace app::classes::types
