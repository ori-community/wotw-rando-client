#pragma once
#include <Modloader/app/structs/SubscriptionModel__Array.h>
#include <Modloader/app/structs/SubscriptionModel__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SubscriptionModel__Array {
        inline app::SubscriptionModel__Array__Class** type_info() {
            static app::SubscriptionModel__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SubscriptionModel__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SubscriptionModel__Array__Class* get_class() {
            return il2cpp::get_class<app::SubscriptionModel__Array__Class>(type_info(), "PlayFab.ClientModels", "SubscriptionModel[]");
        }
        inline app::SubscriptionModel__Array* create() {
            return il2cpp::create_object<app::SubscriptionModel__Array>(get_class());
        }
    } // namespace SubscriptionModel__Array
} // namespace app::classes::types
