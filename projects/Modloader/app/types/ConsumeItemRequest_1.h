#pragma once
#include <Modloader/app/structs/ConsumeItemRequest_1.h>
#include <Modloader/app/structs/ConsumeItemRequest_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace ConsumeItemRequest_1 {
        inline app::ConsumeItemRequest_1__Class** type_info() {
            static app::ConsumeItemRequest_1__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::ConsumeItemRequest_1__Class**)(modloader::win::memory::resolve_rva(0x0475E7F0));
            }
            return cache;
        }
        inline app::ConsumeItemRequest_1__Class* get_class() {
            return il2cpp::get_class<app::ConsumeItemRequest_1__Class>(type_info(), "PlayFab.ServerModels", "ConsumeItemRequest");
        }
        inline app::ConsumeItemRequest_1* create() {
            return il2cpp::create_object<app::ConsumeItemRequest_1>(get_class());
        }
    } // namespace ConsumeItemRequest_1
} // namespace app::classes::types
