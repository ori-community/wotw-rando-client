#pragma once
#include <Modloader/app/structs/SceneDataCaptureAgent_SceneLoadingStates__Enum.h>
#include <Modloader/app/structs/SceneDataCaptureAgent_SceneLoadingStates__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SceneDataCaptureAgent_SceneLoadingStates__Enum {
        inline app::SceneDataCaptureAgent_SceneLoadingStates__Enum__Class** type_info() {
            static app::SceneDataCaptureAgent_SceneLoadingStates__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SceneDataCaptureAgent_SceneLoadingStates__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SceneDataCaptureAgent_SceneLoadingStates__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::SceneDataCaptureAgent_SceneLoadingStates__Enum__Class>(type_info(), "", "SceneDataCaptureAgent", "SceneLoadingStates");
        }
        inline app::SceneDataCaptureAgent_SceneLoadingStates__Enum* create() {
            return il2cpp::create_object<app::SceneDataCaptureAgent_SceneLoadingStates__Enum>(get_class());
        }
    } // namespace SceneDataCaptureAgent_SceneLoadingStates__Enum
} // namespace app::classes::types
