#pragma once
#include <Modloader/app/structs/ConnectedPlayer__Array.h>
#include <Modloader/app/structs/ConnectedPlayer__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace ConnectedPlayer__Array {
        inline app::ConnectedPlayer__Array__Class** type_info() {
            static app::ConnectedPlayer__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::ConnectedPlayer__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::ConnectedPlayer__Array__Class* get_class() {
            return il2cpp::get_class<app::ConnectedPlayer__Array__Class>(type_info(), "PlayFab.MultiplayerModels", "ConnectedPlayer[]");
        }
        inline app::ConnectedPlayer__Array* create() {
            return il2cpp::create_object<app::ConnectedPlayer__Array>(get_class());
        }
    } // namespace ConnectedPlayer__Array
} // namespace app::classes::types
