#pragma once
#include <Modloader/app/structs/Datatype_doubleXdr.h>
#include <Modloader/app/structs/Datatype_doubleXdr__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace Datatype_doubleXdr {
        inline app::Datatype_doubleXdr__Class** type_info() {
            static app::Datatype_doubleXdr__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::Datatype_doubleXdr__Class**)(modloader::win::memory::resolve_rva(0x04761280));
            }
            return cache;
        }
        inline app::Datatype_doubleXdr__Class* get_class() {
            return il2cpp::get_class<app::Datatype_doubleXdr__Class>(type_info(), "System.Xml.Schema", "Datatype_doubleXdr");
        }
        inline app::Datatype_doubleXdr* create() {
            return il2cpp::create_object<app::Datatype_doubleXdr>(get_class());
        }
    } // namespace Datatype_doubleXdr
} // namespace app::classes::types
