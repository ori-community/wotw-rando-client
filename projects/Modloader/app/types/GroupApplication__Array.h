#pragma once
#include <Modloader/app/structs/GroupApplication__Array.h>
#include <Modloader/app/structs/GroupApplication__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace GroupApplication__Array {
        inline app::GroupApplication__Array__Class** type_info() {
            static app::GroupApplication__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::GroupApplication__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::GroupApplication__Array__Class* get_class() {
            return il2cpp::get_class<app::GroupApplication__Array__Class>(type_info(), "PlayFab.GroupsModels", "GroupApplication[]");
        }
        inline app::GroupApplication__Array* create() {
            return il2cpp::create_object<app::GroupApplication__Array>(get_class());
        }
    } // namespace GroupApplication__Array
} // namespace app::classes::types
