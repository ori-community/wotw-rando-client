#pragma once
#include <Modloader/app/structs/CollisionFlags__Enum.h>
#include <Modloader/app/structs/CollisionFlags__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace CollisionFlags__Enum {
        inline app::CollisionFlags__Enum__Class** type_info() {
            static app::CollisionFlags__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::CollisionFlags__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::CollisionFlags__Enum__Class* get_class() {
            return il2cpp::get_class<app::CollisionFlags__Enum__Class>(type_info(), "UnityEngine", "CollisionFlags");
        }
        inline app::CollisionFlags__Enum* create() {
            return il2cpp::create_object<app::CollisionFlags__Enum>(get_class());
        }
    } // namespace CollisionFlags__Enum
} // namespace app::classes::types
