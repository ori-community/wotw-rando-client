#pragma once
#include <Modloader/app/structs/EAppOwnershipFlags__Enum.h>
#include <Modloader/app/structs/EAppOwnershipFlags__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace EAppOwnershipFlags__Enum {
        inline app::EAppOwnershipFlags__Enum__Class** type_info() {
            static app::EAppOwnershipFlags__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::EAppOwnershipFlags__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::EAppOwnershipFlags__Enum__Class* get_class() {
            return il2cpp::get_class<app::EAppOwnershipFlags__Enum__Class>(type_info(), "Steamworks", "EAppOwnershipFlags");
        }
        inline app::EAppOwnershipFlags__Enum* create() {
            return il2cpp::create_object<app::EAppOwnershipFlags__Enum>(get_class());
        }
    } // namespace EAppOwnershipFlags__Enum
} // namespace app::classes::types
