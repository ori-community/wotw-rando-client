#pragma once
#include <Modloader/app/structs/UberShaderModifier__Array.h>
#include <Modloader/app/structs/UberShaderModifier__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace UberShaderModifier__Array {
        inline app::UberShaderModifier__Array__Class** type_info() {
            static app::UberShaderModifier__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::UberShaderModifier__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::UberShaderModifier__Array__Class* get_class() {
            return il2cpp::get_class<app::UberShaderModifier__Array__Class>(type_info(), "", "UberShaderModifier[]");
        }
        inline app::UberShaderModifier__Array* create() {
            return il2cpp::create_object<app::UberShaderModifier__Array>(get_class());
        }
    } // namespace UberShaderModifier__Array
} // namespace app::classes::types
