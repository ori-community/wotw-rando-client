...
namespace core::task {
    ...
    void schedule(float seconds, std::function<void()> task) {
    ...
    void schedule_for_next_update(std::function<void()> task) {
        schedule(-timer, task);
    }
    ...
    IL2CPP_INTERCEPT(GameController, void, FixedUpdate, (app::GameController * this_ptr)) {
        ...
        for (auto const& task : task_buffer) {
            schedule(task.seconds, task.callback);
        }
