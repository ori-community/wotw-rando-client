...
namespace core::text {
    ...
    public:
        std::unordered_map<static_text_entry, TextEntry> text_entries;
    ...
    void initialize_shop_slot(const int slot) {
        register_text(static_cast<static_text_entry>(slot), "Empty");
        register_text(static_cast<static_text_entry>(slot + 1), " ");
        register_text(static_cast<static_text_entry>(slot + 2), "Locked");
        register_text(static_cast<static_text_entry>(slot + 3), " ");
        register_text(static_cast<static_text_entry>(slot + 4), "Undiscovered");
        register_text(static_cast<static_text_entry>(slot + 5), "What could it be?");
    }
    ...
    void register_text(const static_text_entry id, std::string_view text) {
        auto& entry = text_data->text_entries[id];
        entry.text.emplace_back(text);
        notify_changed(reactivity::TextDatabaseDependency { id });
    }

    void register_text(const static_text_entry id, const std::wstring_view text) {
        register_text(id, convert_wstring_to_string(std::wstring(text)));
    }

    void clear_text(const static_text_entry id) {
        auto& entry = text_data->text_entries[id];
        entry.text.clear();
    ...
    std::wstring get_text_w(const static_text_entry id, const int i) {
        notify_used(reactivity::TextDatabaseDependency { id });
    ...
    std::vector<std::string> const& get_all_text(const static_text_entry id) {
        notify_used(reactivity::TextDatabaseDependency { id });
        const auto& entry = text_data->text_entries[id];
        return entry.text;
    }

    std::string get_concatenated_text(const static_text_entry id, const std::string_view delimiter) {
        notify_used(reactivity::TextDatabaseDependency { id });
        std::string text;
        const auto& entry = text_data->text_entries[id];
    ...
    std::string_view get_random_text(const static_text_entry id) {
        notify_used(reactivity::TextDatabaseDependency { id });
        const auto& entry = text_data->text_entries[id];
        if (entry.text.empty()) {
            return "";
    ...
    std::string_view get_random_text_with_hash(const static_text_entry id, const std::size_t hash) {
        notify_used(reactivity::TextDatabaseDependency { id });
        const auto& entry = text_data->text_entries[id];
        if (entry.text.empty()) {
            return "";
    ...
    app::MessageProvider* get_provider(const static_text_entry id, const int i) {
        notify_used(reactivity::TextDatabaseDependency{id});
        const auto& entry = text_data->text_entries[id];
    ...
    app::MessageProvider* get_random_provider(const static_text_entry id) {
        auto& entry = text_data->text_entries[id];
