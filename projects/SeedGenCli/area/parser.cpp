...
namespace area
{
    ...
    void parse_expression(ParserCache& cache, std::vector<std::string>& all, std::vector<std::string>& single, const TokenType entry_type)
    {
        auto in_or = false;
        while (cache.it != cache.end)
        {
            ...
            else if (cache.it->type == TokenType::And || cache.it->type == TokenType::Or)
            {
                ...
                {
                    std::cerr << "Unexpected '" << get_token_name(*cache.it) << "' at " << cache.it->line << std::endl;
                    throw std::exception();
                }
    ...
    AreaTree parse(const std::vector<Token>& tokens)
    {
        ...
        try
        {
            while (cache.it != cache.end)
            {
                ...
                {
                    std::cerr << "Unexpected token '" << get_token_name(*cache.it) << "' at " << cache.it->line << std::endl;
                    throw std::exception();
                }
            }
        }
