...
namespace area
{
    namespace tokenizer
    {
        namespace
        {
            ...
            std::pair<u64, Token> create_define_token(std::string_view input, const std::string_view define,
                                                      const TokenType type, bool break_on_newline = false)
            {
    ...
    std::vector<tokenizer::Token> tokenize(const std::string& area_def)
    {
        ...
        while (!input.empty())
        {
            ...
            for (auto tokenizer : tokenizer::tokenizers)
            {
                auto [consumed, token] = tokenizer(input);
                if (token.type == tokenizer::TokenType::Error)
                {
                    // TODO: Add line and character logging.
                    std::cerr << token.value << "' at line " << line << std::endl;
                    return std::vector<tokenizer::Token>();
                }

                if (token.type == tokenizer::TokenType::Indent)
                {
                    ++line;
                    // Take off redundant indent tokens.
                    if (!tokens.empty() && tokens.back().type == tokenizer::TokenType::Indent)
                        tokens.pop_back();
                }

                if (token.type != tokenizer::TokenType::Null)
                {
                    token.line = line;
