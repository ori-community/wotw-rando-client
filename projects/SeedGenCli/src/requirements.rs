    ...
    pub fn is_met(&self, player: &Player, orbs: Orbs) -> Option<Vec<Orbs>> {
        match self {
            ...
            Requirement::Skill(skill) =>
                if player.has(&Item::Skill(*skill), 1) { return Some(vec![Orbs::default()]); },
            ...
            Requirement::Resource(resource, amount) =>
                if player.has(&Item::Resource(*resource), *amount) { return Some(vec![Orbs::default()]); },
            Requirement::Shard(shard) =>
                if player.has(&Item::Shard(*shard), 1) { return Some(vec![Orbs::default()]); },
            Requirement::Teleporter(teleporter) =>
                if player.has(&Item::Teleporter(*teleporter), 1) { return Some(vec![Orbs::default()]); },
                ...
                if let Some(weapon) = player.preferred_weapon(false) {
                    ...
                    for (enemy, amount) in enemies {
                        ...
                        if enemy.shielded() {
                            ...
                            else if player.has(&Item::Skill(Skill::Grenade), 1) { energy -= player.use_cost(Skill::Grenade) * f32::from(*amount); }
                            else if player.has(&Item::Skill(Skill::Spear), 1) { energy -= player.use_cost(Skill::Spear) * f32::from(*amount); }
                            else { return None; }
                        }
