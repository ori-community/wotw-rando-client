...
impl Requirement {
    fn cost_is_met(&self, cost: f32, player: &Player, orbs: &Orbs, pathsets: &[Pathset]) -> Option<Vec<Orbs>> {
        if orbs.energy >= cost { return Some(vec![
            Orbs {
                energy: -cost,
                ..Default::default()
            }
        ])} else if pathsets.contains(&Pathset::Unsafe) && player.has(Item::Shard(Shard::LifePact), 1) && orbs.energy + orbs.health >= cost { return Some(vec![
            Orbs {
        ...
        match self {
            ...
            Requirement::EnergySkill(skill, amount) =>
                if player.has(Item::Skill(*skill), 1) {
                    ...
                    return self.cost_is_met(cost, player, orbs, pathsets);
                }
                ...
                if let Some(weapon) = player.preferred_weapon(pathsets, false) {
                    ...
                    for (enemy, amount) in enemies {
                        if let Enemy::EnergyRefill = enemy {
                            ...
                            energy = player.max_energy(pathsets).min(energy + *amount as f32);
                            continue;
                        }
...
mod tests {
    ...
    fn is_met() {
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Skill(Skill::Blaze), 1);
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_some());
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs {energy: -1.0, ..orbs }]));
        player.grant(Item::Resource(Resource::Energy), 2);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {energy: -2.0, ..orbs }]));
        ...
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {health: -30.0, ..orbs }]));

        player = Default::default();
        let req = Requirement::BreakWall(12.0);
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Skill(Skill::Sword), 1);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {..orbs }]));
        player = Default::default();
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths).is_none());
        player.grant(Item::Resource(Resource::Energy), 4);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs {energy: -2.0, ..orbs }]));
        player.grant(Item::Resource(Resource::Energy), 6);
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Resource(Resource::Energy), 2);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {energy: -6.0, ..orbs }]));
        ...
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs {energy: -3.25, ..orbs }]));
        player.grant(Item::Resource(Resource::Energy), 6);
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Skill(Skill::DoubleJump), 1);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {energy: -6.5, ..orbs }]));
        player = Default::default();
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths).is_none());
        player.grant(Item::Resource(Resource::Energy), 1);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs {energy: -14.0, ..orbs }]));
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Resource(Resource::Energy), 1);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {energy: -33.0, ..orbs }]));
        player = Default::default();
        ...
        assert!(req.is_met(&player, &player.max_orbs(&moki), &moki).is_none());
        player.grant(Item::Resource(Resource::Energy), 1);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &moki), Some(vec![Orbs {energy: -8.0, ..orbs }]));
        ...
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { health: -10.0, energy: -1.0 }]));
        let req = Requirement::Or(vec![a.clone(), b.clone()]);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { energy: -2.0, ..orbs }, Orbs { health: -20.0, ..orbs }]));
        let req = Requirement::Or(vec![Requirement::And(vec![a.clone(), b.clone()]), Requirement::And(vec![c.clone(), d.clone()]), a.clone(), b.clone()]);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { energy: -1.0, health: -10.0 }, Orbs { energy: -2.0, ..orbs }, Orbs { health: -20.0, ..orbs }]));
        let req = Requirement::And(vec![Requirement::Or(vec![a.clone(), d.clone()]), Requirement::Or(vec![b.clone(), c.clone()])]);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { energy: -1.0, health: -10.0 }]));
        ...
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { energy: -6.0, ..orbs }, Orbs { energy: -4.0, health: -10.0 }, Orbs { health: -60.0, ..orbs }, Orbs { energy: -1.0, health: -40.0 }, Orbs { energy: -2.0, health: -20.0 }]));
        let req = Requirement::Or(vec![Requirement::Free, b.clone()]);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Default::default()]));
        ...
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { energy: -1.0, health: -30.0 }]));
        ...
        player.grant(Item::Skill(Skill::Blaze), 1);
        player.grant(Item::Resource(Resource::Energy), 2);
        assert_eq!(req.is_met(&player, &player.max_orbs(&moki), &unsafe_paths), Some(vec![Orbs { health: -20.0, ..orbs }, Orbs { health: -10.0, energy: -1.0 }, Orbs { energy: -2.0, ..orbs }]));
    }
}
