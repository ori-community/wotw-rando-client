...
package SeedGenerator {
  ...
  case class Teleporter(teleporterId: Int) extends Item with Merch with Important {
    ...
    override val cost: Double = Math.max(6d + (if(Settings.flags.randomSpawn && Settings.unsafePaths) 94d else 0d), Teleporter.costs.getOrElse(teleporterId, 0d))
  }

  object Teleporter {
    val itemType: Int = 5
    val costs: Map[Int, Double] = Map(3 -> 14, 16 -> 14, 17->10)
