...
package SeedGenerator {
  ...
  object FXGUI extends JFXApp {
    ...
    object FXSettingsProvider extends SettingsProvider {
      ...
      override def headers: Seq[String] = FXGUI.Headers.active ++ FXGUI.customHeader().filterNot(_ == "// Replace this text with a seed header, if desired") ++ (if(!settings().flags.noHints && !settings().headerList.exists(_.endsWith("hints"))) Seq(
        ...
        "3|0|8|2|102|int|1500                            // price initialization for ^",
        "6|1000|8|2|102|int|-500                         // price reduction: Bash -500",
        "6|1097|8|2|102|int|-500                         // price reduction: Bow -500",
        "2|113=500|8|2|102|int|0                         // set price to zero if nothing left to hint",
      ) else Nil)
    }
