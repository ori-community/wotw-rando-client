    ...
    FilterResult should_show_icon_with_current_filter(app::AreaMapIconManager* manager, app::RuntimeWorldMapIcon* icon) {
        ...
        if (filter <= NewFilters::Collectibles) {
            ...
            if (is_custom_filter_icon) {
                auto it = custom_filter_icon_states.find(stringify_guid(icon->fields.Guid));
                if (it != custom_filter_icon_states.end()) {
                    const auto transparency = core::settings::map_icon_transparency();
...
RANDOMIZER_C_DLLEXPORT void remove_icon(app::GameWorldAreaID__Enum area, int id) {
...
RANDOMIZER_C_DLLEXPORT void add_icon(app::GameWorldAreaID__Enum area, int id, app::WorldMapIconType__Enum icon, float x, float y, UberStateGroup group_id, int state_id, bool allow_teleport) {
    auto& area_header_icons = header_icons[area];
...
RANDOMIZER_C_DLLEXPORT void set_icon_label(app::GameWorldAreaID__Enum area, int id, const wchar_t* label) {
    auto& area_header_icons = header_icons[area];
    auto it = area_header_icons.find(id);
    if (it != area_header_icons.end()) {
...
RANDOMIZER_C_DLLEXPORT void clear_icons() {
    if (initialized) {
...
RANDOMIZER_C_DLLEXPORT void refresh_map() {
...
RANDOMIZER_C_DLLEXPORT void set_start_in_logic_filter(bool value) {
