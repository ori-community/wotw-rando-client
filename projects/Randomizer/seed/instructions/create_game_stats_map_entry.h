...
INSTRUCTION(CreateGameStatsMapEntry)
    explicit CreateGameStatsMapEntry(const std::string& label, const std::string& icon_name) :
        label(label),
        icon_name(icon_name) {}
    ...
    static std::unique_ptr<IInstruction> from_json(const nlohmann::json& j) {
        return std::make_unique<CreateGameStatsMapEntry>(j.at(0).get<std::string>(), j.at(1).get<std::string>());
    }
};
