...
RANDOMIZER_C_DLLEXPORT void refresh_wheel();
...
RANDOMIZER_C_DLLEXPORT void set_wheel_sticky(int wheel, bool value) {
    wheels[wheel].sticky = value;
}

RANDOMIZER_C_DLLEXPORT void set_wheel_behavior(int behavior) {
    wheel_behavior = static_cast<WheelBehavior>(behavior);
}

// For cleanup.
RANDOMIZER_C_DLLEXPORT void clear_wheels() {
    custom_wheel_input = false;
    custom_wheel_on = false;
    wheel_index = 0;
    wheels.clear();
