...
namespace randomizer::features::wheel {
    namespace {
        ...
        IL2CPP_INTERCEPT(bool, EquipmentWheel, HasElement, app::EquipmentType__Enum type) {
            if (custom_wheel_on) {
        ...
        void binding_callback(CustomWheelEntry const& entry, app::SpellUIItem* item, app::SpellInventory_Binding__Enum binding) {
            ...
            auto callback = entry.callbacks[static_cast<int>(binding)];
            if (callback == nullptr) {
                return;
            }

            callback(wheel_index, index, binding);
        ...
        bool is_valid_wheel_index(int wheel, int item) { return wheel >= 0 && item >= 0 && item < 12; }
    ...
    void set_wheel_sticky(int wheel, bool value) { wheels[wheel].sticky = value; }

    void set_wheel_behavior(int behavior) { wheel_behavior = static_cast<WheelBehavior>(behavior); }
