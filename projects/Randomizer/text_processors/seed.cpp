    ...
    namespace {
        ...
        std::optional<std::string> goal_mode_progress(core::text::ITextProcessor const& base_processor, std::string_view content) {
            ...
            if (std::ranges::find(flags, "All Trees") != flags.end()) {
                goals.push_back(std::format("Trees: {}/14", core::api::uber_states::UberState(UberStateGroup::RandoVirtual, 502).get<int>()));
            }

            if (std::ranges::find(flags, "All Wisps") != flags.end()) {
                goals.push_back(std::format("Wisps: {}/5", core::api::uber_states::UberState(UberStateGroup::RandoVirtual, 503).get<int>()));
            }
