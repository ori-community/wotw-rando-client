...
namespace randomizer::archipelago::ids {
    ...
    [[maybe_unused]] auto on_location_collection_loaded = event_bus().register_handler(
        ...
        EventTiming::After,
        [](auto, auto) {
            //if (!location_map.empty() & should_use_ap_client()) {
    ...
    std::variant<Location, BooleanItem, ResourceItem, UpgradeItem> get_item(archipelago_id_t id) {
        modloader::info("archipelago", std::format("Parsing ID {}", id));
        modloader::info("archipelago", std::format("Type {}, group {}, state {}", static_cast<uint8_t>((id >> 32) % 4), static_cast<uint16_t>(id >> 16), static_cast<uint16_t>(id)));
