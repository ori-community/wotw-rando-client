#include <Modloader/app/il2cpp_internals.h>
#ifdef IL2CPP_STRUCT_SqlXml__StaticFields_INITIALIZING
#elif defined(IL2CPP_STRUCTS_INITIALIZING)
#define IL2CPP_STRUCT_SqlXml__StaticFields_INITIALIZING
#if !defined(IL2CPP_STRUCT_SqlXml__StaticFields_DEFINED)
#define IL2CPP_STRUCT_SqlXml__StaticFields_DEFINED
struct Func_4_System_IO_Stream_System_Xml_XmlReaderSettings_System_Xml_XmlParserContext_System_Xml_XmlReader_;
struct XmlReaderSettings;
struct MethodInfo_1;
struct SqlXml__StaticFields {
    struct Func_4_System_IO_Stream_System_Xml_XmlReaderSettings_System_Xml_XmlParserContext_System_Xml_XmlReader_* s_sqlReaderDelegate;
    struct XmlReaderSettings* s_defaultXmlReaderSettings;
    struct XmlReaderSettings* s_defaultXmlReaderSettingsCloseInput;
    struct MethodInfo_1* s_createSqlReaderMethodInfo;
};
#endif
#if !defined(IL2CPP_STRUCT_SqlXml__StaticFields_FWDDECL)
#define IL2CPP_STRUCT_SqlXml__StaticFields_FWDDECL
#include <Modloader/app/structs/Func_4_Syste_I_Strea_Syste_Xm_XmlReaderSetting_Syste_Xm_XmlParserContex_Syste_Xm_XmlReade_.h>
#include <Modloader/app/structs/MethodInfo_1.h>
#include <Modloader/app/structs/XmlReaderSettings.h>
#endif
#undef IL2CPP_STRUCT_SqlXml__StaticFields_INITIALIZING
#if !defined(IL2CPP_STRUCT_SqlXml__StaticFields_DEFINED) && !defined(IL2CPP_STRUCT_SqlXml__StaticFields_FWDDECL)
#include <Modloader/app/structs/SqlXml__StaticFields.h>
#endif
#else
namespace app {
#define IL2CPP_STRUCTS_INITIALIZING
#include <Modloader/app/structs/SqlXml__StaticFields.h>
#undef IL2CPP_STRUCTS_INITIALIZING
} // namespace app
#endif
