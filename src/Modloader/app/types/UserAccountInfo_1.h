#pragma once
#include <Modloader/app/structs/UserAccountInfo_1.h>
#include <Modloader/app/structs/UserAccountInfo_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace UserAccountInfo_1 {
        inline app::UserAccountInfo_1__Class** type_info() {
            static app::UserAccountInfo_1__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::UserAccountInfo_1__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::UserAccountInfo_1__Class* get_class() {
            return il2cpp::get_class<app::UserAccountInfo_1__Class>(type_info(), "PlayFab.ServerModels", "UserAccountInfo");
        }
        inline app::UserAccountInfo_1* create() {
            return il2cpp::create_object<app::UserAccountInfo_1>(get_class());
        }
    } // namespace UserAccountInfo_1
} // namespace app::classes::types
