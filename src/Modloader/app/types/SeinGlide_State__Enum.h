#pragma once
#include <Modloader/app/structs/SeinGlide_State__Enum.h>
#include <Modloader/app/structs/SeinGlide_State__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SeinGlide_State__Enum {
        inline app::SeinGlide_State__Enum__Class** type_info() {
            static app::SeinGlide_State__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SeinGlide_State__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SeinGlide_State__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::SeinGlide_State__Enum__Class>(type_info(), "", "SeinGlide", "State");
        }
        inline app::SeinGlide_State__Enum* create() {
            return il2cpp::create_object<app::SeinGlide_State__Enum>(get_class());
        }
    } // namespace SeinGlide_State__Enum
} // namespace app::classes::types
