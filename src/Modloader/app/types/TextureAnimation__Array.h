#pragma once
#include <Modloader/app/structs/TextureAnimation__Array.h>
#include <Modloader/app/structs/TextureAnimation__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace TextureAnimation__Array {
        inline app::TextureAnimation__Array__Class** type_info() {
            static app::TextureAnimation__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::TextureAnimation__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::TextureAnimation__Array__Class* get_class() {
            return il2cpp::get_class<app::TextureAnimation__Array__Class>(type_info(), "", "TextureAnimation[]");
        }
        inline app::TextureAnimation__Array* create() {
            return il2cpp::create_object<app::TextureAnimation__Array>(get_class());
        }
    } // namespace TextureAnimation__Array
} // namespace app::classes::types
