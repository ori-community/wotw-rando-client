#pragma once
#include <Modloader/app/structs/AlphaCombineMode__Enum.h>
#include <Modloader/app/structs/AlphaCombineMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace AlphaCombineMode__Enum {
        inline app::AlphaCombineMode__Enum__Class** type_info() {
            static app::AlphaCombineMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::AlphaCombineMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::AlphaCombineMode__Enum__Class* get_class() {
            return il2cpp::get_class<app::AlphaCombineMode__Enum__Class>(type_info(), "", "AlphaCombineMode");
        }
        inline app::AlphaCombineMode__Enum* create() {
            return il2cpp::create_object<app::AlphaCombineMode__Enum>(get_class());
        }
    } // namespace AlphaCombineMode__Enum
} // namespace app::classes::types
