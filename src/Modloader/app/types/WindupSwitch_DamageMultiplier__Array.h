#pragma once
#include <Modloader/app/structs/WindupSwitch_DamageMultiplier__Array.h>
#include <Modloader/app/structs/WindupSwitch_DamageMultiplier__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace WindupSwitch_DamageMultiplier__Array {
        inline app::WindupSwitch_DamageMultiplier__Array__Class** type_info() {
            static app::WindupSwitch_DamageMultiplier__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::WindupSwitch_DamageMultiplier__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::WindupSwitch_DamageMultiplier__Array__Class* get_class() {
            return il2cpp::get_class<app::WindupSwitch_DamageMultiplier__Array__Class>(type_info(), "", "WindupSwitch+DamageMultiplier[]");
        }
        inline app::WindupSwitch_DamageMultiplier__Array* create() {
            return il2cpp::create_object<app::WindupSwitch_DamageMultiplier__Array>(get_class());
        }
    } // namespace WindupSwitch_DamageMultiplier__Array
} // namespace app::classes::types
