#pragma once
#include <Modloader/app/structs/XGameSaveBlob__Array.h>
#include <Modloader/app/structs/XGameSaveBlob__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace XGameSaveBlob__Array {
        inline app::XGameSaveBlob__Array__Class** type_info() {
            static app::XGameSaveBlob__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::XGameSaveBlob__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::XGameSaveBlob__Array__Class* get_class() {
            return il2cpp::get_class<app::XGameSaveBlob__Array__Class>(type_info(), "XGamingRuntime", "XGameSaveBlob[]");
        }
        inline app::XGameSaveBlob__Array* create() {
            return il2cpp::create_object<app::XGameSaveBlob__Array>(get_class());
        }
    } // namespace XGameSaveBlob__Array
} // namespace app::classes::types
