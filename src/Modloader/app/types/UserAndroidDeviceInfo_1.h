#pragma once
#include <Modloader/app/structs/UserAndroidDeviceInfo_1.h>
#include <Modloader/app/structs/UserAndroidDeviceInfo_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace UserAndroidDeviceInfo_1 {
        inline app::UserAndroidDeviceInfo_1__Class** type_info() {
            static app::UserAndroidDeviceInfo_1__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::UserAndroidDeviceInfo_1__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::UserAndroidDeviceInfo_1__Class* get_class() {
            return il2cpp::get_class<app::UserAndroidDeviceInfo_1__Class>(type_info(), "PlayFab.ServerModels", "UserAndroidDeviceInfo");
        }
        inline app::UserAndroidDeviceInfo_1* create() {
            return il2cpp::create_object<app::UserAndroidDeviceInfo_1>(get_class());
        }
    } // namespace UserAndroidDeviceInfo_1
} // namespace app::classes::types
