#pragma once
#include <Modloader/app/structs/TransparencyAnimator_AnimateMode__Enum.h>
#include <Modloader/app/structs/TransparencyAnimator_AnimateMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace TransparencyAnimator_AnimateMode__Enum {
        inline app::TransparencyAnimator_AnimateMode__Enum__Class** type_info() {
            static app::TransparencyAnimator_AnimateMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::TransparencyAnimator_AnimateMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::TransparencyAnimator_AnimateMode__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::TransparencyAnimator_AnimateMode__Enum__Class>(type_info(), "Moon.Timeline", "TransparencyAnimator", "AnimateMode");
        }
        inline app::TransparencyAnimator_AnimateMode__Enum* create() {
            return il2cpp::create_object<app::TransparencyAnimator_AnimateMode__Enum>(get_class());
        }
    } // namespace TransparencyAnimator_AnimateMode__Enum
} // namespace app::classes::types
