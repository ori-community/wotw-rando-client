#pragma once
#include <Modloader/app/structs/PointerEventData__Array.h>
#include <Modloader/app/structs/PointerEventData__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace PointerEventData__Array {
        inline app::PointerEventData__Array__Class** type_info() {
            static app::PointerEventData__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::PointerEventData__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::PointerEventData__Array__Class* get_class() {
            return il2cpp::get_class<app::PointerEventData__Array__Class>(type_info(), "UnityEngine.EventSystems", "PointerEventData[]");
        }
        inline app::PointerEventData__Array* create() {
            return il2cpp::create_object<app::PointerEventData__Array>(get_class());
        }
    } // namespace PointerEventData__Array
} // namespace app::classes::types
