#pragma once
#include <Modloader/app/structs/GetFileMetadata__Array.h>
#include <Modloader/app/structs/GetFileMetadata__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace GetFileMetadata__Array {
        inline app::GetFileMetadata__Array__Class** type_info() {
            static app::GetFileMetadata__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::GetFileMetadata__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::GetFileMetadata__Array__Class* get_class() {
            return il2cpp::get_class<app::GetFileMetadata__Array__Class>(type_info(), "PlayFab.DataModels", "GetFileMetadata[]");
        }
        inline app::GetFileMetadata__Array* create() {
            return il2cpp::create_object<app::GetFileMetadata__Array>(get_class());
        }
    } // namespace GetFileMetadata__Array
} // namespace app::classes::types
