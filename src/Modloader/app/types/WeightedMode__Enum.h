#pragma once
#include <Modloader/app/structs/WeightedMode__Enum.h>
#include <Modloader/app/structs/WeightedMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace WeightedMode__Enum {
        inline app::WeightedMode__Enum__Class** type_info() {
            static app::WeightedMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::WeightedMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::WeightedMode__Enum__Class* get_class() {
            return il2cpp::get_class<app::WeightedMode__Enum__Class>(type_info(), "UnityEngine", "WeightedMode");
        }
        inline app::WeightedMode__Enum* create() {
            return il2cpp::create_object<app::WeightedMode__Enum>(get_class());
        }
    } // namespace WeightedMode__Enum
} // namespace app::classes::types
