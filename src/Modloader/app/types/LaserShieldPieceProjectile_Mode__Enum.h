#pragma once
#include <Modloader/app/structs/LaserShieldPieceProjectile_Mode__Enum.h>
#include <Modloader/app/structs/LaserShieldPieceProjectile_Mode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace LaserShieldPieceProjectile_Mode__Enum {
        inline app::LaserShieldPieceProjectile_Mode__Enum__Class** type_info() {
            static app::LaserShieldPieceProjectile_Mode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::LaserShieldPieceProjectile_Mode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::LaserShieldPieceProjectile_Mode__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::LaserShieldPieceProjectile_Mode__Enum__Class>(type_info(), "", "LaserShieldPieceProjectile", "Mode");
        }
        inline app::LaserShieldPieceProjectile_Mode__Enum* create() {
            return il2cpp::create_object<app::LaserShieldPieceProjectile_Mode__Enum>(get_class());
        }
    } // namespace LaserShieldPieceProjectile_Mode__Enum
} // namespace app::classes::types
