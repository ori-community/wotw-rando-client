#pragma once
#include <Modloader/app/structs/XmlName__Array.h>
#include <Modloader/app/structs/XmlName__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace XmlName__Array {
        inline app::XmlName__Array__Class** type_info() {
            static app::XmlName__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::XmlName__Array__Class**)(modloader::win::memory::resolve_rva(0x04771D10));
            }
            return cache;
        }
        inline app::XmlName__Array__Class* get_class() {
            return il2cpp::get_class<app::XmlName__Array__Class>(type_info(), "System.Xml", "XmlName[]");
        }
        inline app::XmlName__Array* create() {
            return il2cpp::create_object<app::XmlName__Array>(get_class());
        }
    } // namespace XmlName__Array
} // namespace app::classes::types
