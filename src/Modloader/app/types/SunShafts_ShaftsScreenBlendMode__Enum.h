#pragma once
#include <Modloader/app/structs/SunShafts_ShaftsScreenBlendMode__Enum.h>
#include <Modloader/app/structs/SunShafts_ShaftsScreenBlendMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SunShafts_ShaftsScreenBlendMode__Enum {
        inline app::SunShafts_ShaftsScreenBlendMode__Enum__Class** type_info() {
            static app::SunShafts_ShaftsScreenBlendMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SunShafts_ShaftsScreenBlendMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SunShafts_ShaftsScreenBlendMode__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::SunShafts_ShaftsScreenBlendMode__Enum__Class>(type_info(), "UnityStandardAssets.ImageEffects", "SunShafts", "ShaftsScreenBlendMode");
        }
        inline app::SunShafts_ShaftsScreenBlendMode__Enum* create() {
            return il2cpp::create_object<app::SunShafts_ShaftsScreenBlendMode__Enum>(get_class());
        }
    } // namespace SunShafts_ShaftsScreenBlendMode__Enum
} // namespace app::classes::types
