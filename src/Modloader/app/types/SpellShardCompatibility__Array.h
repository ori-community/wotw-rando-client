#pragma once
#include <Modloader/app/structs/SpellShardCompatibility__Array.h>
#include <Modloader/app/structs/SpellShardCompatibility__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace SpellShardCompatibility__Array {
        inline app::SpellShardCompatibility__Array__Class** type_info() {
            static app::SpellShardCompatibility__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::SpellShardCompatibility__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::SpellShardCompatibility__Array__Class* get_class() {
            return il2cpp::get_class<app::SpellShardCompatibility__Array__Class>(type_info(), "", "SpellShardCompatibility[]");
        }
        inline app::SpellShardCompatibility__Array* create() {
            return il2cpp::create_object<app::SpellShardCompatibility__Array>(get_class());
        }
    } // namespace SpellShardCompatibility__Array
} // namespace app::classes::types
