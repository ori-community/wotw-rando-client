#pragma once
#include <Modloader/app/structs/DeflateStream_1.h>
#include <Modloader/app/structs/DeflateStream_1__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace DeflateStream_1 {
        inline app::DeflateStream_1__Class** type_info() {
            static app::DeflateStream_1__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::DeflateStream_1__Class**)(modloader::win::memory::resolve_rva(0x04796D28));
            }
            return cache;
        }
        inline app::DeflateStream_1__Class* get_class() {
            return il2cpp::get_class<app::DeflateStream_1__Class>(type_info(), "Unity.IO.Compression", "DeflateStream");
        }
        inline app::DeflateStream_1* create() {
            return il2cpp::create_object<app::DeflateStream_1>(get_class());
        }
    } // namespace DeflateStream_1
} // namespace app::classes::types
