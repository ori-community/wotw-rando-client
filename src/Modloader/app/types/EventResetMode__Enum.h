#pragma once
#include <Modloader/app/structs/EventResetMode__Enum.h>
#include <Modloader/app/structs/EventResetMode__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace EventResetMode__Enum {
        inline app::EventResetMode__Enum__Class** type_info() {
            static app::EventResetMode__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::EventResetMode__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::EventResetMode__Enum__Class* get_class() {
            return il2cpp::get_class<app::EventResetMode__Enum__Class>(type_info(), "System.Threading", "EventResetMode");
        }
        inline app::EventResetMode__Enum* create() {
            return il2cpp::create_object<app::EventResetMode__Enum>(get_class());
        }
    } // namespace EventResetMode__Enum
} // namespace app::classes::types
