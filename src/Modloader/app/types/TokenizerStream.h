#pragma once
#include <Modloader/app/structs/TokenizerStream.h>
#include <Modloader/app/structs/TokenizerStream__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace TokenizerStream {
        inline app::TokenizerStream__Class** type_info() {
            static app::TokenizerStream__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::TokenizerStream__Class**)(modloader::win::memory::resolve_rva(0x0476F168));
            }
            return cache;
        }
        inline app::TokenizerStream__Class* get_class() {
            return il2cpp::get_class<app::TokenizerStream__Class>(type_info(), "System.Security.Util", "TokenizerStream");
        }
        inline app::TokenizerStream* create() {
            return il2cpp::create_object<app::TokenizerStream>(get_class());
        }
    } // namespace TokenizerStream
} // namespace app::classes::types
