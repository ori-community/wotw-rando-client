#pragma once
#include <Modloader/app/structs/XdrBuilder_XdrEntry__Array.h>
#include <Modloader/app/structs/XdrBuilder_XdrEntry__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace XdrBuilder_XdrEntry__Array {
        inline app::XdrBuilder_XdrEntry__Array__Class** type_info() {
            static app::XdrBuilder_XdrEntry__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                cache = (app::XdrBuilder_XdrEntry__Array__Class**)(modloader::win::memory::resolve_rva(0x04754F00));
            }
            return cache;
        }
        inline app::XdrBuilder_XdrEntry__Array__Class* get_class() {
            return il2cpp::get_class<app::XdrBuilder_XdrEntry__Array__Class>(type_info(), "System.Xml.Schema", "XdrBuilder+XdrEntry[]");
        }
        inline app::XdrBuilder_XdrEntry__Array* create() {
            return il2cpp::create_object<app::XdrBuilder_XdrEntry__Array>(get_class());
        }
    } // namespace XdrBuilder_XdrEntry__Array
} // namespace app::classes::types
