#pragma once
#include <Modloader/app/structs/ConsoleColor__Enum.h>
#include <Modloader/app/structs/ConsoleColor__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace ConsoleColor__Enum {
        inline app::ConsoleColor__Enum__Class** type_info() {
            static app::ConsoleColor__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::ConsoleColor__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::ConsoleColor__Enum__Class* get_class() {
            return il2cpp::get_class<app::ConsoleColor__Enum__Class>(type_info(), "System", "ConsoleColor");
        }
        inline app::ConsoleColor__Enum* create() {
            return il2cpp::create_object<app::ConsoleColor__Enum>(get_class());
        }
    } // namespace ConsoleColor__Enum
} // namespace app::classes::types
