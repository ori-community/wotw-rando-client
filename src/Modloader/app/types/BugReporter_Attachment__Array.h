#pragma once
#include <Modloader/app/structs/BugReporter_Attachment__Array.h>
#include <Modloader/app/structs/BugReporter_Attachment__Array__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace BugReporter_Attachment__Array {
        inline app::BugReporter_Attachment__Array__Class** type_info() {
            static app::BugReporter_Attachment__Array__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::BugReporter_Attachment__Array__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::BugReporter_Attachment__Array__Class* get_class() {
            return il2cpp::get_class<app::BugReporter_Attachment__Array__Class>(type_info(), "", "BugReporter+Attachment[]");
        }
        inline app::BugReporter_Attachment__Array* create() {
            return il2cpp::create_object<app::BugReporter_Attachment__Array>(get_class());
        }
    } // namespace BugReporter_Attachment__Array
} // namespace app::classes::types
