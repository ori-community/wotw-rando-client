#pragma once
#include <Modloader/app/structs/WwiseSoundBankRefCountProfilingSetting.h>
#include <Modloader/app/structs/WwiseSoundBankRefCountProfilingSetting__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace WwiseSoundBankRefCountProfilingSetting {
        inline app::WwiseSoundBankRefCountProfilingSetting__Class** type_info() {
            static app::WwiseSoundBankRefCountProfilingSetting__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::WwiseSoundBankRefCountProfilingSetting__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::WwiseSoundBankRefCountProfilingSetting__Class* get_class() {
            return il2cpp::get_class<app::WwiseSoundBankRefCountProfilingSetting__Class>(type_info(), "Moon.Telemetry.Performance.Settings", "WwiseSoundBankRefCountProfilingSetting");
        }
        inline app::WwiseSoundBankRefCountProfilingSetting* create() {
            return il2cpp::create_object<app::WwiseSoundBankRefCountProfilingSetting>(get_class());
        }
    } // namespace WwiseSoundBankRefCountProfilingSetting
} // namespace app::classes::types
