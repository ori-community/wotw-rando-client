#pragma once
#include <Modloader/app/structs/BlurOptimized_BlurType__Enum.h>
#include <Modloader/app/structs/BlurOptimized_BlurType__Enum__Class.h>
#include <Modloader/il2cpp_helpers.h>
#include <Modloader/macros.h>

namespace app::classes::types {
    namespace BlurOptimized_BlurType__Enum {
        inline app::BlurOptimized_BlurType__Enum__Class** type_info() {
            static app::BlurOptimized_BlurType__Enum__Class** cache = nullptr;
            if (cache == nullptr) {
                static app::BlurOptimized_BlurType__Enum__Class* type_info_ref = nullptr;
                cache = &type_info_ref;
            }
            return cache;
        }
        inline app::BlurOptimized_BlurType__Enum__Class* get_class() {
            return il2cpp::get_nested_class<app::BlurOptimized_BlurType__Enum__Class>(type_info(), "UnityStandardAssets.ImageEffects", "BlurOptimized", "BlurType");
        }
        inline app::BlurOptimized_BlurType__Enum* create() {
            return il2cpp::create_object<app::BlurOptimized_BlurType__Enum>(get_class());
        }
    } // namespace BlurOptimized_BlurType__Enum
} // namespace app::classes::types
