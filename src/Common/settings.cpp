...
namespace common::settings {
    ...
    std::unordered_map<key_t, values_t>& Settings::overrides() {
        return m_overrides;
    }

    bool Settings::get_boolean(std::string_view name, bool default_value) {
        const auto it = m_overrides.find(key_t{name});
        const nlohmann::json_pointer<std::string> path(std::format("/{}", name));
    ...
    int Settings::get_int(std::string_view name, int default_value) {
        const auto it = m_overrides.find(key_t{name});
        const nlohmann::json_pointer<std::string> path(std::format("/{}", name));
        return it != m_overrides.end()
    ...
    float Settings::get_float(std::string_view name, float default_value) {
        const auto it = m_overrides.find(key_t{name});
        const nlohmann::json_pointer<std::string> path(std::format("/{}", name));
        return it != m_overrides.end()
    ...
    std::string Settings::get_string(std::string_view name, std::string const& default_value) {
        const auto it = m_overrides.find(key_t{name});
        const nlohmann::json_pointer<std::string> path(std::format("/{}", name));
        return it != m_overrides.end()
