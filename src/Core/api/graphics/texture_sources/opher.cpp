...
#include <Modloader/app/methods/WeaponmasterScreen.h>
...
namespace {
    ...
    IL2CPP_INTERCEPT(void, WeaponmasterScreen, Init, app::WeaponmasterScreen* this_ptr) {
        next::WeaponmasterScreen::Init(this_ptr);

        for (int i = 0; i < this_ptr->fields.WeaponmasterItems->max_length; ++i) {
            const auto source_texture = this_ptr->fields.WeaponmasterItems->vector[i]->fields.Upgrade->fields.Icon;
