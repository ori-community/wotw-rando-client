...
namespace {
    ...
    IL2CPP_INTERCEPT(void, MapmakerScreen, Init, app::MapmakerScreen* this_ptr) {
        ...
        for (int i = 0; i < this_ptr->fields.Purchases->max_length; ++i) {
            const auto source_texture = this_ptr->fields.Purchases->vector[i]->fields.Icon;
    ...
    auto on_modloader_injection_complete = modloader::event_bus().register_handler(ModloaderEvent::InjectionComplete, [](auto) {
        core::api::graphics::textures::register_source("Lupo", [](const std::string& id) -> std::optional<app::Texture*> {
