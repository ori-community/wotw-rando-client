...
bool load_json_file(const std::filesystem::path& path, nlohmann::json& j) {
    ...
    if (stream.is_open()) {
        try {
            ...
            warn("util", std::format("failed to parse '{}' error '{}' at byte '{}'", path.string(), ex.id, ex.byte));
            return false;
        }
    } else {
        warn("util", std::format("failed to open '{}'", path.string()));
        return false;
    }
