...
INTERCEPT(6645664,void, NPCMessageBox__FixedUpdate, (NPCMessageBox_o* this_ptr), {
    ...
    if (this_ptr->MessageBoxW != npcBox && isVisible(this_ptr->MessageBoxW)) {
//        debug("(index " + std::to_string(this_ptr->MessageBoxW->MessageIndex)+ ") found interactable: " + std::to_string((__int64)this_ptr->MessageBoxW));
        npcBox = this_ptr->MessageBoxW;
    }
})
